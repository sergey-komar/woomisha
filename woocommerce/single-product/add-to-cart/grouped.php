...
<div class="group-product-wrap table-responsive mt-40 mt-sm-26 mt-md-26">
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						case 'quantity':
							...
							$value = ob_get_clean();
							$my_class = 'product-quantity';//мой код
							break;
						...
						case 'label':
							...
							$value .= '</label>';
							$my_class = 'product-list';//мой код
							break;


						case 'price':
							$value = $grouped_product_child->get_price_html() . wc_get_stock_html( $grouped_product_child );
							$my_class = 'product-price';//мой код
							break;
						default:
							$value = '';
							$my_class = '';//мой код
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<div class="quantity-btn-group">
			...
			<button type="submit" class="single_add_to_cart_button button alt btn btn-black<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
			</div>
		</div>
